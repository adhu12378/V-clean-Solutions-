...
function VCleanPage() {
  ...
  return (
    <main className="min-h-screen overflow-hidden bg-background text-foreground">
      <header className="fixed inset-x-0 top-0 z-40 border-b border-border/70 bg-background/80 backdrop-blur-2xl">
        <div className="mx-auto flex h-20 max-w-7xl items-center justify-between px-5 lg:px-8">
          <button className="flex items-center" onClick={() => scrollTo("home")} aria-label="VClean Solutions home">
            <BrandLogo className="h-10 w-[114px]" />
          </button>
          ...
          <div className="hidden items-center gap-3 xl:flex">
            <a href="tel:+447429099670" className="glass-panel flex items-center gap-2 rounded-full px-4 py-2 text-sm font-semibold"><Phone className="size-4 text-accent" />+44 7429 099670</a>
      ...
      <section id="home" className="relative bg-hero pt-32 pb-24 text-primary-foreground md:pt-40 md:pb-32">
        <div className="hero-grid ambient-stars absolute inset-0 opacity-25" />
        <div className="relative mx-auto max-w-7xl px-5 lg:px-8">
          <div className="grid items-center gap-16 lg:grid-cols-[0.88fr_1.12fr]">
            <div className="animate-fade-up">
              <div className="glass-panel mb-7 inline-flex items-center gap-2 rounded-full px-4 py-2 text-xs font-bold uppercase"><Sparkles className="size-4 text-mint" /> Professional Cleaning Services</div>
              <h1 className="max-w-3xl font-display text-5xl font-bold leading-[0.98] sm:text-6xl lg:text-7xl">Every space,<br/><span className="text-electric">wiped spotless.</span></h1>
              ...
              <div className="mt-9 flex flex-col gap-3 sm:flex-row">
                <Button size="lg" className="h-13 px-7" onClick={() => scrollTo("book")}>Book a Clean <ArrowRight /></Button>
                <Button size="lg" variant="outline" className="h-13 px-7 text-primary-foreground" onClick={() => scrollTo("book")}>Calculate Instant Quote</Button>
              </div>
            ...
            <div className="relative grid h-[470px] grid-cols-3 gap-3 sm:gap-5">
              {heroCards.map((card, i) => <article key={card.title} className={`glass-panel group relative h-[390px] overflow-hidden rounded-lg animate-float-${i + 1} ${card.className}`}><img src={card.image} alt={card.title} width={1200} height={912} className="h-full w-full object-cover transition-transform duration-700 group-hover:scale-105" /><div className="absolute inset-x-0 bottom-0 bg-image-fade p-3 sm:p-5"><span className="text-[10px] font-bold uppercase text-accent">{card.label}</span><h2 className="mt-1 font-display text-base sm:text-xl">{card.title}</h2></div></article>)}
              <div className="glass-panel absolute -bottom-2 left-[30%] flex items-center gap-3 rounded-full p-3 text-card-foreground"><span className="grid size-9 place-items-center rounded-full bg-accent/15 text-accent"><span className="size-2 rounded-full bg-accent shadow-[0_0_14px_currentColor]" /></span><span><strong className="block text-sm">Crew available today</strong><small className="text-muted-foreground">4.9 from 280+ reviews</small></span></div>
            </div>
          </div>
      ...
      <section id="why-us" className="scroll-mt-24 border-b border-border bg-background py-20">
        <div className="mx-auto max-w-7xl px-5 lg:px-8">
          ...
          <div className="mt-12 grid gap-3 sm:grid-cols-2 lg:grid-cols-5">{features.map(({ icon: Icon, title, text }) => <article key={title} className="glass-panel rounded-lg p-6"><span className="mb-8 grid size-11 place-items-center rounded-full bg-accent/10 text-accent"><Icon /></span><h3 className="font-display text-lg">{title}</h3><p className="mt-2 text-sm leading-6 text-muted-foreground">{text}</p></article>)}</div>
      ...
      <section id="areas" className="scroll-mt-20 bg-soft py-24 text-primary-foreground">
...
function FooterGroup({ title, children }: { title: string; children: React.ReactNode }) { return <div><h3 className="mb-5 text-xs font-bold uppercase text-accent">{title}</h3><div className="flex flex-col gap-3 text-sm text-primary-foreground/65 [&_a]:flex [&_a]:items-center [&_a]:gap-2 [&_a]:transition-colors [&_a:hover]:text-mint [&_span]:flex [&_span]:items-center [&_span]:gap-2 [&_svg]:size-4">{children}</div></div>; }